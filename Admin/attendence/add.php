...
$stmt=$db->prepare('SELECT * FROM `Class_scedule` where Subject_Code=:sub');
...
$data=$stmt->fetch(PDO::FETCH_OBJ);
 ...
 $sql1=$db->prepare('INSERT into `Attendence` values(?,?,?,?,?,?,?)');
 	...
 	$sql1->bindParam('2',$data->Faculty_id,PDO::PARAM_STR);
...
for ( $j = 1; $j < $num_rows+1; $j ++ ) {
        ...
        $sql->bindParam('3',$data->Faculty_id,PDO::PARAM_STR);
